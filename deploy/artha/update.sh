#!/usr/bin/env bash
# Auto-update pipeline for Artha Accounting (erpsaas fork, Laravel 12 + Filament 3).
#
# Tracks upstream andrewdwallo/erpsaas (branch 3.x) and ships new commits to the
# live Scalingo app with the thin Artha white-label layer preserved — the same
# fork-tracks-upstream model used by Artha's other rebranded repos.
#
# Because the rebrand is a set of isolated git commits (not a hand-rewrite), the
# vast majority of upstream commits merge cleanly. If a merge conflicts, or if a
# merged commit overwrites a branded file, the pipeline STOPS LOUDLY (no deploy)
# and asks for a human — production is never touched in that case.
#
# Flow:
#   1. fetch upstream 3.x
#   2. stop if already up to date (unless --force)
#   3. (dry-run) merge onto a throwaway branch to detect conflicts, run the
#      rebrand assertions and a real asset build, then discard — proves the next
#      upstream is safe to ship without mutating the branch or production
#   4. (real)   merge upstream into 3.x, run rebrand assertions
#   5. rebuild Vite assets (npm ci && npm run build) and commit them
#   6. push 3.x to the GitHub fork
#   7. deploy the archive to Scalingo (top-level artha-accounting/ prefix)
...
UPSTREAM_BRANCH="3.x"
FORK_BRANCH="3.x"
SCALINGO_APP="artha-accounting"
SCALINGO_REGION="osc-fr1"
LIVE_URL="https://artha-accounting.osc-fr1.scalingo.io"
VERIFY_PATH="/company/login"
EXPECTED_TOKEN="Artha Accounting"
...
# working tree must be clean before we touch branches
...
TARBALL="$(mktemp -d)/artha-accounting.tar.gz"
git archive --format=tar.gz --prefix=artha-accounting/ HEAD -o "$TARBALL"
...
git commit -m "Record deployed upstream erpsaas SHA $UPSTREAM_SHA"
git push origin "$FORK_BRANCH"
log "DONE. Artha Accounting updated to upstream $UPSTREAM_SHA and live."
